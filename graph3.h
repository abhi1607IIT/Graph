...
class Graph {
	...
	bool hasPathBFS(char* name1, char* name2) {
		vertex* v1 = findVertex(name1);
		vertex* v2 = findVertex(name2);
		if (v1 == NULL || v2 == NULL) {
			return false;
		...
		while (!q.empty()) {
			...
			q.pop();
			if (current == v2) {
				vertex* currentParent = parent[v2];
				while (currentParent != NULL) {
					cout << currentParent->name << endl;
					currentParent = parent[currentParent];
				}
				return true;
			}
			visited.insert(current);
			vector<vertex*>* adjacent = current->getAdjacentVertices();
			for (int i = 0; i < adjacent->size(); i++) {
				if (visited.count(adjacent->at(i)) == 0) {
					q.push(adjacent->at(i));
					parent[adjacent->at(i)] = current;
				}
			}
		}	
		return false;
